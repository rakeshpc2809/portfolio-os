...
class GoldDampenerCalculatorTest {
    ...
    @Test
    @DisplayName("Midpoint linear taper (dev = 10%): buy = 100% (1.00), sell = 100% (1.00)")
    void testMidpointLinearTaperMultipliers() {
        ...
        assertEquals(1.00, mults.buyMultiplier(), 0.0001, "At dev=10%, buy multiplier must taper linearly to 100%");
        assertEquals(1.00, mults.sellMultiplier(), 0.0001, "At dev=10%, sell multiplier must taper linearly to 100%");
    }

    @Test
    @DisplayName("Extended state (dev >= 20%): buy multiplier = 70% (0.70), sell multiplier = 140% (1.40)")
    void testExtendedStateMultipliers() {
        ...
        assertEquals(0.70, mults.buyMultiplier(), 0.0001);
        ...
        assertEquals(0.70, overMults.buyMultiplier(), 0.0001);
    ...
    void testFloorBackstopOverridesDampenerUnderExtendedState() {
        ...
        // Normal buy allocation at dev=+20% would damp buy to 70%: (5% * 10L) * 0.70 = 35,000
        ...
        assertEquals(new BigDecimal("35000.00"), normalDampenedBuy);
